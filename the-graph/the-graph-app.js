...
var hotKeys = {
  ...
  // shift to toggle the isShiftHeld boolean
  16: function(app) {
...
module.exports.register = function(context) {
  ...
  TheGraph.App = React.createFactory(
    createReactClass({
      ...
      nodeInBox: function(node, boxStart, boxEnd) {
        var x = node.metadata.x;
        var y = node.metadata.y;
        return x > boxStart.x && x < boxEnd.x && y > boxStart.y && y < boxEnd.y;
      },
      createBoxSelection: function(event) {
        // TODO: add in graphical display of the box selection
        // TODO: add the nodes inside the box selection to the selectedNodes array
        if (this.boxSelectionStart === null) {
          this.boxSelectionStart = event.gesture.center;
        }
        var selected = [];
        for (var nodeIndex in this.refs.graph.props.graph.nodes) {
          var node = this.refs.graph.props.graph.nodes[nodeIndex];
          if (this.nodeInBox(node, this.boxSelectionStart, event.gesture.center)){
            selected.push(node);
          }
        }
      ...
      render: function() {
        ...
        var scaleClass = sc > TheGraph.zbpBig
          ? "big"
          : sc > TheGraph.zbpNormal ? "normal" : "small";
